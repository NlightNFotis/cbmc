...
std::vector<codet> require_goto_statements::get_all_statements(
  const exprt &function_value)
{
  ...
  std::copy_if(function_value.depth_begin(), function_value.depth_end(),
  ...
  for(const auto &assignment : statements)
  {
    if(assignment.get_statement() == ID_assign)
    {
      ...
      if(code_assign.lhs().id() == ID_member)
      {
        ...
        if(symbol.get_identifier()==structure_name&&
...
require_goto_statements::pointer_assignment_locationt require_goto_statements::find_pointer_assignments(
  ...
  const std::vector<codet> &entry_point_instructions)
{
  ...
  throw no_decl_found_exception(id2string(variable_name));
}
